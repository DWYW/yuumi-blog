...
import { setGlobalState } from "@/micro-application"
...
const mutations = {
  [MUTATION_TYPE.UPDATE_TOKEN]: function(state: GlobalState, token: string) {
    ...
    if (token !== $storage.local.getItem(ACCESS_TOKEN)) {
      $storage.local.setItem(ACCESS_TOKEN, token)
    }
    setGlobalState(state)
  },
  [MUTATION_TYPE.UPDATE_USER_DATA]: function(state: GlobalState, value: User) {
    state.user = value
    if (JSON.stringify(value) !== $storage.local.getItem(SIGNIN_USER)) {
      $storage.local.setItem(SIGNIN_USER, value)
    }
