...
func (model *Model) DeleteOne(ctx context.Context, condition *DeleteCondition) (*schema.Article, error) {
	dest := schema.Article{}
	result := model.GetDB().WithContext(ctx).Scopes(condition.Build()...).Delete(&dest)
	return &dest, result.Error
}

func (model *Model) DeleteWithID(ctx context.Context, id int64) (*schema.Article, error) {
